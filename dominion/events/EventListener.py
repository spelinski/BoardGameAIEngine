...
class EventListener(object):
    ...
    def notify(self, notification):
        ...
        if notification.type == "played-cards":
        	broadcast_message(self.players, CommandGenerator().create_player_played_message(self.number, notification.cards))
        if notification.type == "trashed-cards":
        	broadcast_message(self.players, CommandGenerator().create_player_trashed_message(self.number, notification.cards))
        if notification.type == "discard-card":
        	broadcast_message(self.players, CommandGenerator().create_player_discard_message(self.number, notification.card))
        if notification.type == "revealed-cards":
        	broadcast_message(self.players, CommandGenerator().create_player_reveal_message(self.number, notification.cards))
